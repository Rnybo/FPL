...
Simplifications (all small-magnitude, don't affect the safe-vs-haul decision):
...
- expected_bonus and minor_pts_fixed (cards, pen-miss/save, conceded-goals penalty)
  are added deterministically per sample, not simulated -- bonus is a relative
  in-match rank per Layer 5's own justification, and the others are too rare/small
  to be worth their own distributions.
...
def top_captain_picks(conn, gameweek: int, candidates_top_n: int = 40, n_samples: int = 10000, seed: int = 0, top_k: int = 5):
    ...
    safe = result_df.nlargest(top_k, "mean")[["name", "mean", "p10", "p90", "p_haul", "p_blank"]]
    haul = result_df.nlargest(top_k, "p_haul")[["name", "mean", "p10", "p90", "p_haul", "p_blank"]]
    return safe, haul
